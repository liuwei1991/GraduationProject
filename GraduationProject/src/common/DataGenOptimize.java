...
public class DataGenOptimize {
	...
	public DataGenOptimize(int num,int keyLen,int columnNum,String filePath){
		this.num = num;
		this.keyLen = keyLen;
	...
	public void genData() throws IOException{
		...
		for(int i=0;i<this.num;i++){
			...
			for(int j=0;j<keyLen;j++){
				line += String.valueOf(r.nextInt(10));
			}
			for(int j=0;j<this.columnNum;j++){
				line+=" "+r.nextInt(10000);
			}
			bw.write(line+"\r\n");
			totalNum++;
		}
	...
	public static void main(String[] args) throws IOException{
		...
		for(int len:keylen){
			for(int i=1;i<=10;i++){
				DataGenOptimize dg = new DataGenOptimize(500*i*base,len,columnNum,"D:/TestData/t2/keylen="+len+" columnNum="+columnNum+"/"+500*i+"w");
				if(threadNum<=1){
					dg.genData();
				}else{
